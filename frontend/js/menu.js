  ...
  $("dd > a").click(function (evt) {
	...
	$.ajax({
        ...
        obj.forEach( function(speech) {
          $(target).append("<div class='row'><div class='small-12 large-6 columns'><p>" +speech.datum+" "+speech.start+" : "+speech.title+"</div><div class='small-12 large-6 columns'><a href='#' class='button openmodal' style='width: 64%; padding: 0.001rem 0rem' data-reveal-id='infos"+speech.number+"-"+target.slice(1,target.length)+"'> weitere Informationen</a></p></div></div>");
    ...
    if (!ajaxLoading) {
      ...
      $.ajax({
        ...
        console.log(obj);
        obj.forEach( function(speech) {
          $(target).append("<div class='row'><div class='small-12 large-6 columns'><p>"+speech.start+" : "+speech.title+"</div><div class='small-12 large-6 columns'><form action='../backend/teilnehmen.php' method='get'><input type=hidden id=title name=title value="+speech.title+"><a href='#' class='button openmodal' style='width: 64%; padding: 0.001rem 0rem' data-reveal-id='infos"+speech.number+"-"+target.slice(1,target.length)+"'> weitere Informationen</a> <input type='submit' id='filter' class='button' style='width: 35%; padding: 0.001rem 0rem' value='Vormerken'></form></div></p></div>");
